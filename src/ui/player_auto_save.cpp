/*
    ...
    $Id: player_auto_save.cpp 276 2006-11-20 21:16:05Z goblinhack $
*/
...
static string gh_ui_display_player_auto_save (gh_game *game,
                                             vector<string> files,
                                             size_t base,
                                             size_t at,
                                             const size_t how_many)
{
    ...
    if (!files.size()) {
        font1->printat(2, "No Mortals Saved Yet!");
        return ("");
    }

    font1->printat(2, "Continue Game?");
...
void gh_myscreen::display (void)
{
    ...
    chose = gh_ui_display_player_auto_save(game, files, base, at, how_many);
...
gh_event_ret gh_ui_new_player_auto_save_loop (gh_game *game)
{
    GH_FUNCTION()

    screen.game = game;
    screen.ret_code = GH_UI_EVENT_NONE;
    screen.playerdir = gh_global::locatedir("saved");
    screen.base = 0;
    screen.at = 0;
    screen.how_many = 10;
    screen.chose = "";
    screen.files = gh_global::listdir_recursive(screen.playerdir, "auto_save");

    if (!screen.files.size()) {
