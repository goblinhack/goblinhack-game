/*
    ...
    $Id: level_start.cpp 1275 2009-08-07 13:01:53Z Ghack $
*/
...
void gh_myscreen::display (void)
{
    ...
    if (cnt > 120) {
        ...
        font1->printat(my - 9, game->player->thing->level->desc.c_str());
    }

    if (cnt > 240) {
        gh_term_fg(GH_TERM_COLOR_WHITE);
        font1->printat(my, "Pray, Mortal!");
    } else if (cnt > 180) {
        if (cnt == 240) {
            gh_audio::play("Zoom_Down_FX_001");
        }

        gh_term_fg(GH_TERM_COLOR_WHITE);
        font1->printat(my, "Pray, Mortal! 3...");
    } else if (cnt > 120) {
        gh_term_fg(GH_TERM_COLOR_YELLOW);
        font1->printat(my, "Pray, Mortal! 2...");
    } else if (cnt > 60) {
        gh_term_fg(GH_TERM_COLOR_GREEN);
        font1->printat(my, "Pray, Mortal! 1...");
    } else if (cnt) {
        gh_term_fg(GH_TERM_COLOR_RED);
        font1->printat(my, "Mwuuhahah!");
    }

    if (!cnt) {
        gh_global::done = true;
        ignore_game_start_events_ticks = 10;
    }
}
...
int gh_myscreen::key_down (const SDL_keysym & sym)
{
    ...
    gh_audio::halt("Zoom_Down_FX_001");
    return (false);
}
...
gh_event_ret gh_ui_level_start_loop (gh_game *game)
{
    GH_FUNCTION()

    if (game->player->thing->level->level_no > 1) {
