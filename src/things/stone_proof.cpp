...
int gh_thing::stone_proof (void)
{
    ...
    //
    int proofing = _stone_proof + stone_proof_boost;
...
//
bool gh_thing::is_stone_proof (void)
{
    GH_FUNCTION()

    return (gh_math::rand100() < stone_proof());
}

//
// Give the thing a boost of "stone_proof", that fades away
//
void gh_thing::stone_proof_boost_start (int boost)
{
    GH_FUNCTION()
    ...
    stone_proof_boost += boost;
}

//
// If a boost is in operation, gradually ease off of its effects.
//
void gh_thing::stone_proof_boost_tick (void)
{
    GH_FUNCTION()

    if (stone_proof_boost > 0) {
        stone_proof_boost--;
    } else if (stone_proof_boost < 0) {
        stone_proof_boost++;
    }
    if (stone_proof_boost == 0) {
        stone_proof_boost_stop();
    }
}
...
//
void gh_thing::stone_proof_boost_stop (void)
{
    GH_FUNCTION()
}
