...
int gh_thing::water_proof (void)
{
    ...
    //
    int proofing = _water_proof + water_proof_boost;
...
//
bool gh_thing::is_water_proof (void)
{
    GH_FUNCTION()

    return (gh_math::rand100() < water_proof());
}

//
// Give the thing a boost of "water_proof", that fades away
//
void gh_thing::water_proof_boost_start (int boost)
{
    GH_FUNCTION()
    ...
    water_proof_boost += boost;
}

//
// If a boost is in operation, gradually ease off of its effects.
//
void gh_thing::water_proof_boost_tick (void)
{
    GH_FUNCTION()

    if (water_proof_boost > 0) {
        water_proof_boost--;
    } else if (water_proof_boost < 0) {
        water_proof_boost++;
    }
    if (water_proof_boost == 0) {
        water_proof_boost_stop();
    }
}
...
//
void gh_thing::water_proof_boost_stop (void)
{
    GH_FUNCTION()
}
