...
int gh_thing::cold_proof (void)
{
    ...
    //
    int proofing = _cold_proof + cold_proof_boost;
...
//
bool gh_thing::is_cold_proof (void)
{
    GH_FUNCTION()

    return (gh_math::rand100() < cold_proof());
}

//
// Give the thing a boost of "cold_proof", that fades away
//
void gh_thing::cold_proof_boost_start (int boost)
{
    GH_FUNCTION()
    ...
    cold_proof_boost += boost;
}

//
// If a boost is in operation, gradually ease off of its effects.
//
void gh_thing::cold_proof_boost_tick (void)
{
    GH_FUNCTION()

    if (cold_proof_boost > 0) {
        cold_proof_boost--;
    } else if (cold_proof_boost < 0) {
        cold_proof_boost++;
    }
    if (cold_proof_boost == 0) {
        cold_proof_boost_stop();
    }
}
...
//
void gh_thing::cold_proof_boost_stop (void)
{
    GH_FUNCTION()
}
