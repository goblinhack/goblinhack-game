...
It is an old and fairly well documented trick to use cellular automata
 < /index.php?title=Cellular_automata > to generate hell-like structures.
...
static int GH_MAP_WIDTH                 = 32;
static int GH_MAP_HEIGHT                = 32;
...
// Grow our cells
//
void gh_maze::hell_generation (void)
{
    GH_FUNCTION()
...
// Generate a maze!
//
void gh_maze::hell_gen (void)
{
    GH_FUNCTION()

    GH_LOG("Generate new hell");

    maze_w = GH_MIN(GH_MAP_WIDTH, GH_MAX_MAZE_SIZE - 2);
    maze_h = GH_MIN(GH_MAP_HEIGHT + (level - 60) * 5, GH_MAX_MAZE_SIZE - 2);
    ...
    //
    // Change the walls to lava and the floor to dirt.
    //
